...
const SpinWheel: React.FC = () => {
  ...
  const loadPrizesAndStatus = async () => {
    try {
      setLoading(true);
      const prizesData = await api.getPrizes();
      setPrizes(prizesData);
      
      // Check if user is authenticated by trying to get rewards
      try {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {!loading && !isAuthenticated && (
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            Please login to spin the wheel and win amazing prizes!
          </p>
          <Link
            to="/login"
      ...
      {/* Wheel Interface - Only show when authenticated */}
      {!loading && isAuthenticated && (
