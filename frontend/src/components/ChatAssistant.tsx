...
const ChatAssistant: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await api.chatWithAssistant([...messages, userMessage]);
      ...
      // If AI recommends products, load them
      if (response.recommended_product_ids && response.recommended_product_ids.length > 0) {
        loadRecommendations(response.recommended_product_ids);
      }
    } catch (err: any) {
      setMessages((prev) => [
        ...prev,
  ...
  const loadRecommendations = async (productIds: number[]) => {
    try {
      // Load products individually since we don't have a bulk endpoint
      const products = await Promise.all(
        productIds.slice(0, 4).map((id) => api.getProduct(id).catch(() => null))
      );
      setRecommendations(products.filter((p): p is Product => p !== null));
    } catch (err) {
