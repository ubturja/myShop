...
class ApiClient {
  ...
  async getCart(): Promise<CartItem[]> {
    const response = await this.client.get<{ cart: CartItem[] }>('/cart');
  ...
  async getGroupBuys(params?: {
    ...
    const response = await this.client.get<{ group_buys: GroupBuy[] }>('/group-buys', {
      params,
    });
    return response.data.group_buys;
  }
