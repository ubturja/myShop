...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->command->info('Importing products from JSON...');
        $this->call(ProductsFromJsonSeeder::class);
        
        // Get all products for group buys
        $products = Product::all();

        // Create inventory records for quick commerce items (if not already created by JSON seeder)
        $this->command->info('Verifying inventory records...');
        $quickItems = $products->where('is_quick_item', true);
        
        foreach ($quickItems as $product) {
            // Skip if inventory already exists (from JSON seeder)
        ...
        foreach ($activeGroupBuys as $groupBuy) {
            // Ensure member count doesn't exceed available customers AND is less than target
            $maxMembers = min($groupBuy->target_size - 1, $customers->count());
            $memberCount = rand(1, max(1, $maxMembers));
            
            if ($memberCount > 0 && $memberCount <= $customers->count()) {
        ...
        $maxFailedMembers = min($failedGroupBuy->target_size - rand(2, 5), $customers->count());
        $memberCount = max(1, $maxFailedMembers);
        
        if ($memberCount > 0 && $memberCount <= $customers->count()) {
            $members = $customers->random(min($memberCount, $customers->count()));
            foreach ($members as $member) {
                GroupBuyMember::create([
